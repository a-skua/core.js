...
declare const resultNumber: ResultInstance<number>;
...
(() => {
  const result: ResultInstance<number, string> = Math.random() > 0.5
    ? ok(1)
    : err("0");
  const value = result.map((n) => n + 1).unwrap((e) => Number(e));
  test<number>(value);
});
...
(() => {
  const result: ResultInstance<number, string> = Math.random() > 0.5
    ? ok(1)
    : err("0");
  const value = result.map((n) => n + 1).unwrap();
  test<number>(value);
});

(() => {
  const result = Math.random() > 0.5 ? ok(1) : err("0");
  const actual = result.map((n) => n + 1).or((e) => ok(e));
  test<ResultInstance<number | string, string>>(actual);
});

(() => {
  const result = Math.random() > 0.5 ? ok<number, string>(1) : err("0");
  const actual = result.map((n) => n + 1).unwrap((e) => Number(e.trim()));
  test<number>(actual);
});
...
(() => {
  const result = Math.random() > 0.5 ? ok(1) : err("0");
  const actual = result.map((n) => n + 1).unwrap();
  test<number>(actual);
});
